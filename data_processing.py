...
def display_time_intervals():
    ...
    data = read_csv_file()
    examination_DateTime = data['examDate'].to_numpy()
...
def del_variables_less_than_33():
    ...
    # 指定要删除的列名称列表
    columns_to_remove = ['id', 'examDate', 'name']
...
def del_third_exam():
    ...
    print(data.keys())
    data['Row_Num'] = data.groupby('id').cumcount()
    # 根据新列进行筛选，删除每组的最后一个数据
    data_new = data[data['Row_Num'] < data.groupby('id')['id'].transform('size') - 1]
...
def interpolation_data():
    ...
    for key in keys:
        if data[key].isnull().sum() != 0:
            data[key] = data.groupby('id')[key].ffill()
            data[key] = data.groupby('id')[key].bfill()
